/**
 * PHASE 4.5A2.2 — OWNER HEADING_TO_PICKUP RELOAD / REOPEN RECOVERY — TEST-ONLY RED PROOF
 *
 * Tests the NEXT lifecycle state after the CERTIFIED `searching` (4.5A1) and
 * `accepted` (4.5A2.1) reload recoveries (do NOT reopen): `heading_to_pickup`.
 *
 ...
 *   Owner  → UI mostra o estado pickup/approach ANTES do reload
 ...
 * restaura automaticamente e exige a MESMA sessão heading_to_pickup de volta
 * na UI. Se a UI pickup/approach NÃO reaparecer sozinha (produto não
 * implementa a recuperação deste estado), é a prova VERMELHA válida — NÃO
 * corrigir aqui.
 *
 * IMPORTANTE — o marcador de UI data-testid="walk-accepted-state" ("A caminho")
 * NÃO identifica sozinho o estado heading_to_pickup. Ele é válido apenas em
 * combinação com a asserção explícita de backend:
 *
 *   current_status === 'heading_to_pickup'
 *
 ...
 *     petwalker_start_heading após o reload,
 *   - petwalker_start_heading NUNCA é invocado por admin/test — apenas pela
 *     UI real do Walker ("Iniciar deslocamento"),
...
// matching ST_DWithin (fixture consistente com os testes certificados 4.4).
...
const log = (msg: string) =>
  console.log(`[${new Date().toISOString()}] [4.5a2-owner-heading-to-pickup-recovery] ${msg}`);
...
test.describe('Phase 4.5A2.2: Owner heading_to_pickup reload recovery (red proof)', () => {
  ...
  test('Owner heading_to_pickup: reload /search-walk NÃO perde a MESMA sessão sem nenhuma ação', async ({ browser }) => {
    runId = `4.5a2_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    const petName = 'PetHeading45A2';
    ...
    try {
      ...
      await test.step('PROVA heading_to_pickup ANTES do reload (backend + UI do Owner)', async () => {
        // Backend: MESMA sessão, current_status heading_to_pickup, MESMO Walker.
        await expect
          .poll(
            async () => {
              const { data } = await admin
        ...
        // UI do Owner: o marcador ESTÁVEL do estado pickup/approach deve estar
        // visível: data-testid="walk-accepted-state" (pill "A caminho" do
        // WalkInProgress em fase pickup — renderiza para accepted E
        // heading_to_pickup enquanto o PIN de retirada não é confirmado).
        // IMPORTANTE: este marcador por si só NÃO identifica
        // heading_to_pickup; ele é válido apenas combinado com a asserção de
        // backend current_status === 'heading_to_pickup' (autoridade do
        // domínio).
        ...
        log('UI do Owner: walk-accepted-state visível ANTES do reload (heading_to_pickup representado + backend confirma)');
      });
      ...
      await test.step('AÇÃO DE RESILIÊNCIA: reload da MESMA página /search-walk', async () => {
        ...
        // atuais de accept_walk_request e petwalker_start_heading — nenhuma
        // chamada nova pode ocorrer após o reload do Owner.
      ...
      await test.step('VERDADE PÓS-RELOAD (ZERO ações do usuário): backend íntegro, sem novas chamadas', async () => {
        ...
        // Verdade autoritativa: MESMA sessão, ainda heading_to_pickup, MESMO
        // Owner, MESMO Walker, MESMO Pet.
        const s = await auditSession(sessionId);
        expect(s.id).toBe(sessionId);
        expect(s.customer_id).toBe(ownerId);
        expect(s.pet_id).toBe(petId);
        ...
        log('pós-reload: banco íntegro (same session/owner/walker/pet, heading_to_pickup, 1 sessão ativa por dono e por pet, zero novas RPCs)');
      });

      await test.step('RECUPERAÇÃO AUTOMÁTICA esperada (RED até o produto restaurar heading_to_pickup)', async () => {
        // COMPORTAMENTO DESEJADO: após o reload, com ZERO ações do usuário, a
        // MESMA sessão heading_to_pickup deve restaurar automaticamente a UI
        // de pickup/approach (marcador walk-accepted-state). O produto hoje
        // NÃO restaura heading_to_pickup (a descoberta automática certificada
        // cobre apenas `searching` + `accepted`; ?resume não aceita
        // heading_to_pickup) — este passo é a prova VERMELHA esperada. NÃO
        // corrigir o produto nesta tarefa.
        await expect(ownerPage!.getByTestId('walk-accepted-state')).toBeVisible({ timeout: 20000 });
        log('RECUPERAÇÃO AUTOMÁTICA CONFIRMADA (verde): mesma sessão heading_to_pickup restaurada sem ação');
      });
    } finally {

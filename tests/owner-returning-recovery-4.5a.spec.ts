/**
 * PHASE 4.5A2.5 — OWNER RETURNING RESUME / RELOAD RECOVERY — TEST-ONLY FIRST PROOF
 *
 * Baseline certificada (NÃO reabrir):
 *   - 4.4 (full-journey-operational-4.4) certificou o lifecycle completo,
 *     incluindo in_progress → request-return-button → customer_request_return
 *     (HTTP 200 + true) → returning (status+current_status) →
 *     owner-returning-state visível → request-return-button AUSENTE →
 *     walker-returning-state visível → GPS continua → confirm chegada →
 *     completed. O GPS de returning (LOC_B) JÁ é certificado e NÃO é
 *     repetido aqui.
 *   - 4.5A recovery GREEN: searching, accepted, heading_to_pickup, arrived,
 *     in_progress (tests/owner-in-progress-recovery-4.5a.spec.ts — TEMPLATE
 *     PRIMÁRIO deste teste).
 *
 * O QUE ESTE TESTE PROVA (4.5A2.5) — apenas a retomada/reload de RETURNING:
 *   A) Auto-discovery normal continua EXATAMENTE searching/accepted/
 *      heading_to_pickup/arrived — returning NÃO é adicionado (arquitetura
 *      intencional; resume flow explícito para in_progress/returning/
 *      completed via ?resume). Nada é testado em /search-walk SEM ?resume.
 *   B) Transição REAL para returning pela UI do Owner: UM clique no
 *      request-return-button → customer_request_return HTTP 200 + body true
 *      (observador RPC certificado) → backend returning.
 *   C) Re-entrada legítima em /inicio → banner REAL "Passeio em andamento"
 *      (ActiveWalkBanner consulta in_progress/returning) → UM clique →
 *      o PRÓPRIO banner cria /search-walk?resume=<MESMA sessionId>.
 *   D) Pré-reload: owner-returning-state + confirm-return-arrival-button
 *      visíveis, request-return-button ausente, backend returning, MESMA
 *      sessão/Owner/Walker/Pet, 1 sessão ativa por dono e por pet.
 *   E) ownerPage.reload({ waitUntil: 'domcontentloaded' }) e ZERO ações:
 *      pathname + ?resume preservados, backend ainda returning (sem
 *      completed, sem regressão a in_progress, sem sessão nova), UI
 *      returning restaura automaticamente, e TODOS os contadores RPC são
 *      comparados ANTES/DEPOIS (monotônicos, sem reset):
 *      create/accept/heading/arrive/confirm_pickup/request_return/
 *      confirm_arrival — reload causa ZERO NOVAS chamadas.
 *      customer_request_return ocorre EXATAMENTE UMA VEZ (UI legítima) e
 *      customer_confirm_arrival permanece ZERO (o teste NÃO avança para
 *      completed — confirm-return-arrival-button NUNCA é clicado).
 *
 * JORNADA REAL até in_progress (réplica certificada 4.5A2.4/4.4 como SETUP):
 *   Owner UI create_walk_request → searching
 *   admin.rpc('process_walk_matching') (scheduler simulado — ÚNICA RPC admin)
 *   Walker UI accept → heading UI → "Cheguei no Local" (GPS real) → arrived
 *   PIN REAL lido da UI do Owner (pickup-pin-display — NUNCA admin/DB)
 *   Walker digita o PIN na UI → petwalker_confirm_pickup (HTTP 200 + reload
 *   real do produto) → backend in_progress + walk-in-progress-marker.
 *
 * OBSERVABILIDADE DA CHEGADA: técnica T6 certificada (armInPageArriveObserver
 * instalada ANTES de walkerCtx.newPage()): exposeBinding + addInitScript,
 * fetch nativo original, response.clone() → clone.text() → binding report →
 * Response ORIGINAL devolvida intocada. ZERO leitores CDP do body da chegada,
 * ZERO barreiras de reload, ZERO preflight de GPS.
 *
 * REGRAS:
 ...
 *   customer_confirm_arrival. ÚNICA exceção certificada:
 *   admin.rpc('process_walk_matching') (scheduler).
 * - NÃO insere walk_sessions nem walk_offers manualmente.
 * - NÃO testa /search-walk sem ?resume (arquitetura intencional).
 * - NÃO testa GPS durante returning (já certificado em 4.4).
...
// Estados ativos (não terminais) do domínio — INCLUI returning para as
// provas de unicidade.
...
const ARRIVE_CONSOLE_FILTER = /arriv|pickup|GPS|geolocation|supabase|fetch|confirm|return/i;

const log = (msg: string) =>
  console.log(`[${new Date().toISOString()}] [4.5a2.5-owner-returning-recovery] ${msg}`);
...
test.describe('Phase 4.5A2.5: Owner returning resume/reload recovery (banner ?resume flow)', () => {
  ...
  // comparados DEPOIS (NUNCA resetados; aprendizado do audit T1 de 4.5A2.4).
  ...
  test('Owner returning: banner REAL /inicio cria ?resume e reload restaura a UI returning sem ação', async ({ browser }) => {
    runId = `4.5a2.5_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    const petName = 'PetReturning45A25';
    ...
    try {
      ...
      await test.step('walker: PIN digitado pela UI → petwalker_confirm_pickup → in_progress', async () => {
        ...
        // Backend: MESMA sessão, status + current_status in_progress.
        await expect
          .poll(
            async () => {
              const s = await auditSession(sessionId);
      ...
      await test.step('SETUP de retomada: banner REAL /inicio → ?resume → UI in_progress', async () => {
        // Re-entrada legítima de produto (o banner é o caminho REAL do produto
        // para a UI de passeio ativo) — NENHUM ?resume injetado pelo teste.
        await ownerPage!.goto('/inicio');
      ...
      await test.step('TRANSIÇÃO REAL para returning: UM clique no request-return-button', async () => {
        // Pré-prova: CTA in_progress visível e customer_request_return ainda
        // NUNCA chamado (contador monotônico === 0).
        ...
        // Contrato CERTIFICADO 4.4: customer_request_return HTTP 200 + true.
        await expect
          .poll(
            () => {
          ...
          .toBeTruthy();
        log('customer_request_return real observado (HTTP 200 + true, via UI "Solicitar retorno")');

        // Backend autoritativo: MESMA sessão, status + current_status returning.
        await expect
          .poll(
            async () => {
              const s = await auditSession(sessionId);
        ...
        // UI do Owner: estado returning canônico.
        ...
        // SUPLEMENTAR (padrão estável 4.4): Walker recebe returning ao vivo.
        await expect(walkerPage!.getByTestId('walker-returning-state')).toBeVisible({ timeout: 20000 });
        log('walker-returning-state visível (prova suplementar, sem GPS)');
      });

      await test.step('RE-ENTRY PROOF: /inicio → banner REAL (backend returning) → ?resume criado pelo banner', async () => {
        // Navegação normal de produto (re-entrada) — NENHUM ?resume injetado.
        await ownerPage!.goto('/inicio');
        await expect(ownerPage!).toHaveURL(/\/inicio/, { timeout: 15000 });

        // Backend continua a MESMA sessão returning MESMO Owner/Walker/Pet.
        const s = await auditSession(sessionId);
        ...
        // Banner REAL (ActiveWalkBanner consulta in_progress/returning).
        const banner = ownerPage!.getByRole('button', { name: /Passeio em andamento/i });
        await expect(banner).toBeVisible({ timeout: 30000 });
        log('Home ActiveWalkBanner visível automaticamente (returning)');
      });
      ...
      await test.step('banner → clique ÚNICO → /search-walk?resume=<sessionId> criado PELO BANNER', async () => {
        ...
        await expect
          ...
          .toBeTruthy();
        log(`banner criou a retomada returning: ${ownerPage!.url()}`);
      });

      await test.step('PROVA pré-reload: UI returning hidratada + backend íntegro (ZERO ações)', async () => {
        // UI returning canônica restaurada pela retomada do próprio produto.
        await expect(ownerPage!.getByTestId('owner-returning-state')).toBeVisible({ timeout: 20000 });
        await expect(ownerPage!.getByTestId('confirm-return-arrival-button')).toBeVisible({ timeout: 15000 });
        // A UI NÃO pode regredir ao CTA in_progress.
        await expect(ownerPage!.getByTestId('request-return-button')).toHaveCount(0);

        // Backend: MESMA sessão returning, MESMO Owner/Walker/Pet.
        const s = await auditSession(sessionId);
        expect(s.id).toBe(sessionId);
        expect(s.customer_id).toBe(ownerId);
        expect(s.walker_id).toBe(walkerId);
        ...
        // customer_confirm_arrival permanece ZERO (não avançamos para
        // completed); customer_request_return ocorreu EXATAMENTE UMA vez.
        const c = lifecycleCounts();
        expect(c.confirmArrival).toBe(0);
        expect(c.returnReq).toBe(1);
        log('pré-reload: UI returning hidratada + backend returning + invariâncias íntegras');
      });

      await test.step('AÇÃO DE RESILIÊNCIA: reload da MESMA página ?resume=<sessionId>', async () => {
        // Contadores MONOTÔNICOS registrados ANTES do reload (todos os 7).
        ...
        // Garantia explícita: a confirmação final de chegada NUNCA ocorreu.
        expect(confirmArrivalCountBeforeReload).toBe(0);
      ...
      await test.step('VERDADE PÓS-RELOAD (ZERO ações do usuário): retomada returning automática intacta', async () => {
        ...
        // 3. Backend: MESMA sessão returning, MESMO Owner/Walker/Pet —
        //    sem completed, sem regressão a in_progress, sem sessão nova.
        const s = await auditSession(sessionId);
        expect(s.id).toBe(sessionId);
        expect(s.customer_id).toBe(ownerId);
        expect(s.walker_id).toBe(walkerId);
        ...
        // 6. ZERO novas RPCs de lifecycle causadas pelo reload — comparação
        //    ANTES/DEPOIS monotônica (arrays NUNCA são resetados).
        ...
        // Invariante CRÍTICA de returning: request_return EXATAMENTE UMA vez
        // (UI legítima) e reload/re-entry causaram ZERO NOVAS solicitações.
        expect(returnRequestCountBeforeReload).toBe(1);
        expect((rpcCalls['customer_request_return'] || []).length).toBe(returnRequestCountBeforeReload);
        // 8. customer_confirm_arrival permanece ZERO — o reload NÃO conclui
        //    o passeio nem confirma chegada.
        ...
        // 7. UI returning restaura automaticamente (ZERO ação) — e a UI NÃO
        //    pode regredir ao CTA in_progress.
        await expect(ownerPage!.getByTestId('owner-returning-state')).toBeVisible({ timeout: 30000 });
        await expect(ownerPage!.getByTestId('confirm-return-arrival-button')).toBeVisible({ timeout: 15000 });
        await expect(ownerPage!.getByTestId('request-return-button')).toHaveCount(0);

        // Backend permanece returning após a restauração da UI (nunca
        // completed; nunca in_progress).
        const s2 = await auditSession(sessionId);
        expect(s2.status).toBe('returning');
        expect(s2.current_status).toBe('returning');
        log('pós-reload: owner-returning-state + confirm-return-arrival-button restaurados + backend returning íntegro + ZERO RPCs duplicadas');
      });

      await test.step('invariante final: MESMA sessão única do run, ainda returning', async () => {
        ...
        expect(runSessions![0].current_status).toBe('returning');
        log('RETURNING_RESUME_RECOVERY_4.5A2.5_PROOF_COMPLETED');
      });
    } finally {
